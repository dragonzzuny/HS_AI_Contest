...
WEB_FALLBACK = os.environ.get("WEB_FALLBACK", "").strip()    # "1"이면 KB 미보유 질문을 공식홈페이지 웹검색으로 폴백
...
DOMAIN_TRIGGERS = {
    ...
    "노무": ["임금", "월급", "급여", "체불", "근로", "노동", "산재", "퇴직금",
            "일자리", "구직", "취업", "채용", "이직", "일거리",
            "job", "employ", "hiring", "hire", "找工作", "求职", "就业", "招聘",
            "việc làm", "tìm việc", "tuyển dụng", "หางาน", "สมัครงาน", "រកការងារ", "ការងារ",
...
# ── 웹 폴백: KB에 없으면 '공식 홈페이지'만 검색해 근거 기반 안내(가드형) ──────
def _is_official(url: str) -> bool:
    """공식 정부·공공 도메인만 허용(블로그·상업 사이트 배제)."""
    try:
    ...
    if not host:
        return False
    return (host.endswith(".go.kr") or host.endswith(".or.kr") or host.endswith(".gov.kr")
            or host in ("gov.kr", "law.go.kr") or host.endswith("liveinkorea.kr"))


async def _web_search_official(query: str, k: int = 3):
    ua = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120 Safari/537.36"}
    async with httpx.AsyncClient(timeout=12, follow_redirects=True, headers=ua) as cli:
        r = await cli.post("https://html.duckduckgo.com/html/", data={"q": query})
    out = []
    for h in re.findall(r'class="result__a"[^>]*href="([^"]+)"', r.text):
        m = re.search(r"uddg=([^&]+)", h)
        real = unquote(m.group(1)) if m else h
        if _is_official(real) and real not in out:
            out.append(real)
        if len(out) >= k:
            break
    return out
...
async def _web_fetch_text(url: str) -> str:
    ua = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120 Safari/537.36"}
    async with httpx.AsyncClient(timeout=12, follow_redirects=True, headers=ua) as cli:
        r = await cli.get(url)
...
WEB_DISCLAIMER = {
    "ko": "⚠️ 이 답변은 공식 홈페이지 인터넷 검색 기반이며, 정확한 대상·조건은 담당기관에 확인하세요.",
    "en": "⚠️ This answer is based on an official-site web search; please confirm eligibility/details with the responsible office.",
    "zh": "⚠️ 此回答基于官方网站检索,具体对象与条件请向主管部门确认。",
    "vi": "⚠️ Câu trả lời dựa trên tìm kiếm trang web chính thức; vui lòng xác nhận đối tượng/điều kiện với cơ quan phụ trách.",
    "th": "⚠️ คำตอบนี้อ้างอิงการค้นหาจากเว็บไซต์ทางการ โปรดยืนยันคุณสมบัติ/เงื่อนไขกับหน่วยงานที่รับผิดชอบ",
    "km": "⚠️ ចម្លើយនេះផ្អែកលើការស្វែងរកគេហទំព័រផ្លូវការ សូមបញ្ជាក់លក្ខខណ្ឌជាមួយស្ថាប័នទទួលបន្ទុក។",
}


async def web_fallback_answer(query: str, lang: str):
    """KB 미보유 질의 → 공식 홈페이지 검색·근거 요약. 실패 시 None(안전 거부로)."""
    if not WEB_FALLBACK:
        return None
    try:
        urls = await _web_search_official(f"{query} 화성 외국인", k=4)
    except Exception:
        return None
    ctx, srcs = "", []
    for u in urls[:2]:
        try:
            txt = await _web_fetch_text(u)
            if len(txt) > 200:
                ctx += f"[출처: {u}]\n{txt}\n\n"
                srcs.append(u)
        except Exception:
    ...
    if not srcs:
        return None
    dis = WEB_DISCLAIMER.get(lang, WEB_DISCLAIMER["ko"])
    if not OLLAMA_HOST:                       # LLM 없으면 지어내지 않고 공식 링크만 안내
        return ("관련 공식 홈페이지에서 확인하세요:\n" + "\n".join(srcs) + "\n\n" + dis), srcs
    sysmsg = (
        "다음은 인터넷 '공식 홈페이지(.go.kr 등)'에서 가져온 내용이다. 이 내용에만 근거해 답하라. "
        "근거에 없는 사실·지명·기관·전화번호를 지어내지 마라. 외국인 대상 여부 등 자격 조건에 특히 주의하고, "
        "확실치 않으면 '담당기관 확인 필요'라고 하라. "
        f"답변은 {LANGS.get(lang, '한국어')}로 간결히 쓰라."
    )
    try:
        async with httpx.AsyncClient(timeout=120) as cli:
            r = await cli.post(f"{OLLAMA_HOST}/api/chat", json={
...
async def chat(req: ChatRequest):
    ...
    # 환각 방지: KB 근거가 없으면 ① 공식 홈페이지 웹 폴백 → ② 그래도 없으면 안전 안내.
    if not is_grounded(req.message, hits):
        web = await web_fallback_answer(req.message, req.lang)
        if web:
            ans, srcs = web
            cits = [{"id": "web", "title": (urlparse(u).hostname or "공식 홈페이지"),
                     "layer": "WEB", "snippet": "인터넷 공식 홈페이지 검색 결과",
                     "dept": "", "phone": "", "source": f"[{u}]({u})",
                     "confidence": "", "kind": "웹"} for u in srcs]
            return JSONResponse({
                "answer": ans, "citations": cits, "actions": [],
                "grounded": False, "refused": False, "served": "web",
                "mode": "ollama" if OLLAMA_HOST else "mock",
            })
